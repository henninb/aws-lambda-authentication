...
aws apigateway put-integration \
    ...
    --type AWS_PROXY \
    --integration-http-method POST \
    --uri "arn:aws:apigateway:{region}:lambda:path/2015-03-31/functions/arn:aws:lambda:{region}:{account-id}:function:$FUNCTION_NAME/invocations"
...
aws lambda add-permission \
    ...
    --action lambda:InvokeFunction \
    --principal apigateway.amazonaws.com \
    --source-arn "arn:aws:execute-api:$REGION:{account-id}:$API_ID/*/POST/api-login2"
...
API_URL="https://${API_ID}.execute-api.{region}.amazonaws.com/${STAGE_NAME}/api-login2"
curl -X POST 'https://api.bhenning.com/api-login2' -d '{"email": "henninb@gmail.com", "password": "monday1"}'
